...
SELECT cust_id, cust_name FROM customer
WHERE SUBSTRING(cust_name, 1, 1) = 'S';
...
SELECT * FROM product
ORDER BY prod_type, prod_id ASC;
...
SELECT prod_id, prod_desc, (prod_quantity * prod_price) AS TOTAL_VALUE FROM product
WHERE prod_quantity > (
SELECT AVG(prod_quantity) FROM product);
...
SELECT rep_id, SUM(cust_balance) AS REP_SUM FROM customer
GROUP BY rep_id
ORDER BY rep_id;
